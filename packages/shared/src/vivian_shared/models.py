...
class CharitableDonationSchema(BaseModel):
    ...
    organization_name: str = Field(description="Name of the charitable organization")
    donation_date: Optional[date] = Field(None, description="Date donation was made")
    ...
    description: Optional[str] = Field(None, description="Description of donation/purpose")
    raw_model_output: Optional[str] = Field(None, description="Raw model output for debugging")


class ParsedReceipt(BaseModel):
    """Result of parsing a receipt."""
    category: ExpenseCategory = Field(default=ExpenseCategory.HSA, description="Receipt category")
    expense: Optional[ExpenseSchema] = Field(None, description="HSA expense data (if category is HSA)")
    charitable_data: Optional[CharitableDonationSchema] = Field(None, description="Charitable donation data (if category is CHARITABLE)")
